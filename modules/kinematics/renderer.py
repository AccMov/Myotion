...
class Renderer:
    ...
    def enableShadows(self, shadowLight, strength=0.5, resolution=[512, 512]):
        ...
        self.shadowObject = Shadow(
            shadowLight, strength=strength, resolution=resolution
        )
    ...
    def render(
        ...
        if self.shadowsEnabled:
            # set render target properties
            glBindFramebuffer(
                GL_FRAMEBUFFER, self.shadowObject.renderTarget.framebufferRef
            )
            glViewport(
                0,
                0,
                self.shadowObject.renderTarget.width,
                self.shadowObject.renderTarget.height,
            )
            ...
            for mesh in meshList:
                ...
                self.shadowObject.material.uniforms["modelMatrix"].data = (
                    mesh.getWorldMatrix()
                )
